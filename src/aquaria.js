...
class AquariaAlignment {
    ...
    setFullSequence (sequenceWidget) {
        ...
        if (
            ...
            soup.structureIds.length === 1
        ) {
            structureId = `${structureId}[1]`
            soup.structureIds[iStructure] = structureId
            console.log(
                'setFullSequence mangle structureId to match alignment',
                soup.structureIds
            )
        }
        ...
        for (let iChain = 0; iChain < chains.length; iChain += 1) {
            ...
            if (isCopy) {
                let prevChains = _.filter(
                    chains.slice(0, iChain),
                    c => c === chain
                )
                let iCopy = prevChains.length + 1
                structureId = `${this.data.pdb_id}[${iCopy}]`
                iStructure = _.findIndex(
                    soup.structureIds,
                    s => s === structureId
                )
            }
            ...
            for (
                ...
                if (iResOfSeq === 0) {
                    for (let iResOfPadding of _.range(
                        sequenceWidget.nPadChar
                    )) {
                        let startLabel = null
                        if (iResOfPadding === 0) {
                            startLabel = ''
                ...
                let pdbRes = this.mapSeqResToPdbResOfChain(
                    seqId,
                    seqResNum,
                    chain,
                    structureId
                ...
                if (_.isNil(pdbRes)) {
                    ...
                    let residue = soup.findFirstResidue(
                        chain,
                        pdbResNum,
                        structureId
                    )
            ...
            for (let iResOfSeq of getIndexes(masterSeq, this.selectSeq)) {
                for (let iRes = iResOfSeq; iRes < iResOfSeq + nSeq; iRes += 1) {
                    ...
                    if (!_.isNil(pdbRes)) {
                        ...
                        let residue = soup.findFirstResidue(chain, pdbResNum)
                        if (!_.isNil(residue)) {
                            this.embedJolecule.controller.setResidueSelect(
                                residue.iRes,
                                true
                            )
                        }
