    ...
    for (let i = iOldPoint; i < jOldPoint - 1; i += 1) {
        ...
        for (let j = jStart; j < jEnd; j += 1) {
            ...
            if (i < oldPath.points.length - 2) {
                ...
                nextOldPoint = oldPath.points[i + 1]
                    .clone()
...
class BufferRibbonGeometry extends THREE.BufferGeometry {
    ...
    countVertexAndFacesOfPath () {
        ...
        for (let trace of this.parameters.traces) {
            ...
            this.nFace +=
                (nTrace - 1) * 2 * this.nShape * (2 * trace.detail + 1)
    ...
    build () {
        for (let iPath of _.range(this.paths.length)) {
            ...
            for (
                ...
                // works out end of expanded path, including overhang
                let iPathEnd =
                    (iTracePoint + 1) * 2 * trace.detail - trace.detail + 1
                if (iPathEnd >= path.points.length) {
                ...
                for (
                    ...
                    let isFront =
                        iPathPoint === 0 && iTracePoint === iTraceStart
                    if (isFront) {
                    ...
                    function getRoundedShapeNormals (iPathPoint) {
                        ...
                        for (let i = 0; i < nVertex; i += 1) {
                            ...
                            let v = shapePoints[i]
                            diffPrev
                                .subVectors(v, shapePoints[iPrev])
                                .normalize()
                            diffNext
                                .subVectors(v, shapePoints[iNext])
                                .normalize()
                            shapeNormal
                                .addVectors(diffPrev, diffNext)
                                .normalize()
                            x = path.normals[iPathPoint]
                                .clone()
                                .multiplyScalar(shapeNormal.x)
...
class BufferRaisedShapesGeometry extends THREE.BufferGeometry {
    ...
    recolor (newColorList) {
        ...
        for (let [
            iVertexSet,
            vertices,
        ] of this.parameters.verticesList.entries()) {
...
function applyMatrix4toVector3array (matrix, vec3Array, iStart, iEnd) {
    ...
    for (let i = iStart; i < iEnd; i += 3) {
        ...
        vec3Array[i + 1] =
            elems[1] * x + elems[5] * y + elems[9] * z + elems[13]
        vec3Array[i + 2] =
            elems[2] * x + elems[6] * y + elems[10] * z + elems[14]
    }
}
...
class CopyBufferGeometry extends THREE.BufferGeometry {
    constructor (copyBufferGeometry, nCopy) {
        ...
        let uvs = expandFloatArray(
            copyBufferGeometry.attributes.uv.array,
            nCopy
        )
