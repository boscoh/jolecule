...
class ResidueProxy {
    ...
    isConnectedToPrev () {
        return (
            this.isProteinConnectedToPrev() ||
            this.isNucleotideConnectedToPrev()
        )
    }
...
class Soup {
    ...
    pushStructureId (structureId, title) {
        ...
        if (title) {
            this.title =
                `[<a href="http://www.rcsb.org/structure/${id}">${id}</a>] ` +
                title
        }
    }
    ...
    areCloseResidues (iRes0, iRes1) {
        ...
        for (let iAtom0 of atomIndices0) {
            for (let iAtom1 of atomIndices1) {
                if (
                    v3.distance(
                        atom0.load(iAtom0).pos,
                        atom1.load(iAtom1).pos
                    ) < 4
                ) {
                    return true
                }
