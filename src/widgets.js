...
class DistanceMeasuresWidget {
    ...
    drawFrame () {
        ...
        if (distances.length > this.distanceMeasures.length) {
            for (
                let i = this.distanceMeasures.length;
                i < distances.length;
                i += 1
            ) {
...
class SequenceWidget extends CanvasWidget {
    ...
    xStructFromIChar (iRes) {
        return parseInt(
            ((iRes - this.iCharStructStart) / this.nCharStruct) *
                this.textWidth()
        )
    }
    ...
    iCharFromXSeq (x) {
        return (
            parseInt((x * this.nCharSeq) / this.textWidth()) +
            this.iCharSeqStart
        )
    }
            ...
            if (_.isNil(chainEntry)) {
                ...
                if (_.isNil(oldChain) && !_.isNil(this.chain)) {
                    ...
                    if (
                        ...
                        let iCharSeqStart = this.findFirstResidue(
                            this.iCharStructStart
                        )
    ...
    draw () {
        ...
        for (
            let iChar = this.iCharSeqStart;
            iChar < this.iCharSeqEnd;
            iChar += 1
        ) {
            let charEntry = this.charEntries[iChar]
        ...
        for (let i = 0; i < soup.getResidueCount(); i += 1) {
            ...
            if (residue.selected) {
                if (chain === null || chain !== residue.chain) {
                    ...
                    chain = residue.chain
                    text +=
                        soup.structureIds[residue.iStructure] +
                        '-' +
                        chain +
                        ': '
                    isDrawComma = false
